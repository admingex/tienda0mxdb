...
$sql="UPDATE CMS_RelPromocionSitioCanal SET posicionIn=0 WHERE id_canalSi=24";
$db -> Execute($sql);
 //ORDENAMOS CONFORME A ESTE NUEVO CRITERIO 
$npos=1;
foreach ($_POST['pe'] as $value){
 	$sql="UPDATE CMS_RelPromocionSitioCanal SET posicionIn=$npos WHERE id_promocionIn=$value AND id_canalSi=24";
	$db -> Execute($sql);
	$npos++;
}
...
if(count($arrayPE) !=0){
	$sql="UPDATE CMS_RelPromocionSitioCanal SET publicadoBi=0 WHERE id_canalSi=24";
	$db -> Execute($sql);
	foreach ($arrayPE as $value){
		$sql="UPDATE CMS_RelPromocionSitioCanal SET publicadoBi=1 WHERE id_promocionIn=$value AND id_canalSi=24";
		$db -> Execute($sql);
	}
}
else{
