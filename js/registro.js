...
function valida_password(acc,pass){
	...
	if(pass.length<8){
		...
		if(!nc.test(pass)){
			$("#password").focus().after("<span class='error'>Sólo debe incluir letras y números</span>");
			return false;			
		}		
		else{			
			if((pass==cadlogin)){
				...
				return false;				
   				...
   				if(!contiene_mayuscula(pass)){
   					$("#password").focus().after("<span class='error2'>Debe contener por lo menos una mayúscula</span>");
   					return false;   					
   				}   			
   				else{
   					if(!contiene_numero(pass)){
   						$("#password").focus().after("<span class='error'>Debe contener por lo menos un número</span>");
   						return false;   						
   					}
   					else{
   						if(!contiene_minuscula(pass)){
   							$("#password").focus().after("<span class='error'>Debe contener por lo menos una minúscula</span>");
   							return false;   							
   						}
   						else{
   							if(!contiene_consecutivos(pass)){
   								$("#password").focus().after("<span class='error2'>No se debe incluir el mismo caracter más de 2 veces</span>");
   								return false;   								
   							}
   							else{
   								return true;
