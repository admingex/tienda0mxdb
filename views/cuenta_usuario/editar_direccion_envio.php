...
<div class='encabezado-descripcion'>Editar dirección de Envío</div>
<div class="form-container">
<form id="form_editar_direccion_envio" action="" method="post">
<div class='form-label'>Calle</div>
<input type="text" name="txt_calle" id="txt_calle" size="30" value="<?php if(isset($_POST['txt_calle'])) echo htmlspecialchars($_POST['txt_calle']); else echo $direccion[0]['calle']; ?>"/>
...
<input type="text" name="txt_numero" id="txt_numero" maxlength="5" size="5" value="<?php if(isset($_POST['txt_numero'])) echo htmlspecialchars($_POST['txt_numero']); else echo $direccion[0]['num_ext']; ?>"/>
<?php if(isset($reg_errores['txt_numero'])) echo ($reg_errores['txt_numero']);?>
<div class="space-label"></div>

<div class='form-label'>N&uacute;mero interior</div>
<input type="text" name="txt_num_int" id="txt_num_int" maxlength="5" size="5" value="<?php if(isset($_POST['txt_num_int'])) echo htmlspecialchars($_POST['txt_num_int']);  else if (isset($direccion[0]['num_int'])) echo $direccion[0]['num_int']; ?>"/>
<?php if(isset($reg_errores['txt_num_int'])) echo ($reg_errores['txt_num_int']);?>
<div class="space-label"></div>

<div class='form-label'>C&oacute;digo postal</div>
<input type="text" name="txt_cp" id="txt_cp" maxlength="5" size="5" value="<?php if(isset($_POST['txt_cp'])) echo htmlspecialchars($_POST['txt_cp']); else echo $direccion[0]['cp']; ?>"/>
<input type="button" id="btn_cp" name="btn_cp" value="llenar" class='boton-guardar'/>
...
<input type="text" name="txt_estado" id="txt_estado" size="30" value="<?php if(isset($_POST['txt_estado'])) echo htmlspecialchars($_POST['txt_estado']); else echo $direccion[0]['estado']; ?>"/>
...
<input type="text" name="txt_ciudad" id="txt_ciudad" size="30" value="<?php if(isset($_POST['txt_ciudad'])) echo htmlspecialchars($_POST['txt_ciudad']); else echo $direccion[0]['ciudad']; ?>"/>
...
<input type="text" name="txt_colonia" id="txt_colonia" size="30" value="<?php if(isset($_POST['txt_colonia'])) echo htmlspecialchars($_POST['txt_colonia']); else echo $direccion[0]['colonia']; ?>"/>
...
<select id="sel_estados" name="sel_estados">
	...
	if (isset($lista_estados_sepomex)) {
		//$clave_estado = "";
		$clave_estado = $direccion[0]['estado'];
...
<select id="sel_ciudades" name="sel_ciudades">
	...
	if (isset($lista_ciudades_sepomex)) {
		//$clave_ciudad = "";
		$clave_ciudad = $direccion[0]['ciudad'];
		...
		if (gettype($lista_ciudades_sepomex) == 'object') {
			...
			{
				if ($clave_ciudad == $ciudad['ciudad']) {
					echo "<option value='". $ciudad['ciudad']. "' selected='true'>".
						$ciudad['ciudad'].
						"</option>";
				} else if ($ciudad['ciudad'] != ''){
					echo "<option value='". $ciudad['ciudad']. "'>".
						$ciudad['ciudad'].
						"</option>";
				}
			}
...
<select id="sel_colonias" name="sel_colonias">
	...
	if (isset($lista_colonias_sepomex)) {
		//$clave_colonia = "";
		$clave_colonia = $direccion[0]['colonia'];
...
<input type="text" name="txt_telefono" id="txt_telefono" size="30" value="<?php if (isset($_POST['txt_telefono'])) echo htmlspecialchars($_POST['txt_telefono']); else echo $direccion[0]['telefono']; ?>" />
<?php if(isset($reg_errores['txt_telefono'])) echo ($reg_errores['txt_telefono']);?>
<div class="space-label"></div>
<div class='form-label'>Referencia</div>
<textarea type="text" name="txt_referencia" id="txt_referencia" cols="30" ><?php if(isset($_POST['txt_referencia'])) echo htmlspecialchars($_POST['txt_referencia']); else if (isset($direccion[0]['referencia'])) echo htmlspecialchars($direccion[0]['referencia']); ?></textarea>
<div class="space-label"></div>					
<div class='form-label'></div>
<div class="check-label"><input type="checkbox" id="chk_default" name="chk_default" <?php if($direccion[0]['estatus'] == 3) echo "checked='TRUE'"?>estatus/>Usar para pago express</div>
<div class="space-label"></div>

<div class='form-label'></div>					
<input type="button" id="guardar_direccion" name="guardar_direccion" value="Guardar"  onclick="enviar_dir_envio(<?php echo $direccion[0]['id_consecutivoSi']?>)" class='boton-guardar'/> ó
